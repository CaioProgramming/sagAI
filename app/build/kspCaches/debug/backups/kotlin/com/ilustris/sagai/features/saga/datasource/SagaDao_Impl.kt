...
public class SagaDao_Impl(
  __db: RoomDatabase,
) : SagaDao {
  private val __db: RoomDatabase

  private val __insertAdapterOfSaga: EntityInsertAdapter<Saga>

  private val __deleteAdapterOfSaga: EntityDeleteOrUpdateAdapter<Saga>

  private val __updateAdapterOfSaga: EntityDeleteOrUpdateAdapter<Saga>
  init {
    this.__db = __db
    this.__insertAdapterOfSaga = object : EntityInsertAdapter<Saga>() {
      protected override fun createQuery(): String =
          "INSERT OR REPLACE INTO `sagas` (`id`,`title`,`description`,`icon`,`createdAt`,`genre`,`mainCharacterId`,`currentActId`,`isEnded`,`endedAt`,`isDebug`,`endMessage`,`emotionalReview`,`introduction`,`playstyle`,`topCharacters`,`actsInsight`,`conclusion`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"

      protected override fun bind(statement: SQLiteStatement, entity: Saga) {
        statement.bindLong(1, entity.id.toLong())
        statement.bindText(2, entity.title)
        statement.bindText(3, entity.description)
        statement.bindText(4, entity.icon)
        statement.bindLong(5, entity.createdAt)
        statement.bindText(6, __Genre_enumToString(entity.genre))
        val _tmpMainCharacterId: Int? = entity.mainCharacterId
        if (_tmpMainCharacterId == null) {
          statement.bindNull(7)
        } else {
          statement.bindLong(7, _tmpMainCharacterId.toLong())
        }
        val _tmpCurrentActId: Int? = entity.currentActId
        if (_tmpCurrentActId == null) {
          statement.bindNull(8)
        } else {
          statement.bindLong(8, _tmpCurrentActId.toLong())
        }
        val _tmp: Int = if (entity.isEnded) 1 else 0
        statement.bindLong(9, _tmp.toLong())
        statement.bindLong(10, entity.endedAt)
        val _tmp_1: Int = if (entity.isDebug) 1 else 0
        statement.bindLong(11, _tmp_1.toLong())
        statement.bindText(12, entity.endMessage)
        val _tmpEmotionalReview: String? = entity.emotionalReview
        if (_tmpEmotionalReview == null) {
          statement.bindNull(13)
        } else {
          statement.bindText(13, _tmpEmotionalReview)
        }
        val _tmpReview: Review? = entity.review
        if (_tmpReview != null) {
          statement.bindText(14, _tmpReview.introduction)
          statement.bindText(15, _tmpReview.playstyle)
          statement.bindText(16, _tmpReview.topCharacters)
          statement.bindText(17, _tmpReview.actsInsight)
          statement.bindText(18, _tmpReview.conclusion)
        } else {
    ...
    this.__deleteAdapterOfSaga = object : EntityDeleteOrUpdateAdapter<Saga>() {
      protected override fun createQuery(): String = "DELETE FROM `sagas` WHERE `id` = ?"

      protected override fun bind(statement: SQLiteStatement, entity: Saga) {
        statement.bindLong(1, entity.id.toLong())
      }
    }
    this.__updateAdapterOfSaga = object : EntityDeleteOrUpdateAdapter<Saga>() {
      protected override fun createQuery(): String =
          "UPDATE OR ABORT `sagas` SET `id` = ?,`title` = ?,`description` = ?,`icon` = ?,`createdAt` = ?,`genre` = ?,`mainCharacterId` = ?,`currentActId` = ?,`isEnded` = ?,`endedAt` = ?,`isDebug` = ?,`endMessage` = ?,`emotionalReview` = ?,`introduction` = ?,`playstyle` = ?,`topCharacters` = ?,`actsInsight` = ?,`conclusion` = ? WHERE `id` = ?"
      ...
      protected override fun bind(statement: SQLiteStatement, entity: Saga) {
        ...
        statement.bindLong(19, entity.id.toLong())
      }
    }
  }

  public override suspend fun saveSagaData(saga: Saga): Long = performSuspending(__db, false, true)
      { _connection ->
    val _result: Long = __insertAdapterOfSaga.insertAndReturnId(_connection, saga)
    _result
  }

  public override suspend fun deleteSagaData(saga: Saga): Unit = performSuspending(__db, false,
      true) { _connection ->
    __deleteAdapterOfSaga.handle(_connection, saga)
  }

  public override suspend fun updateSaga(saga: Saga): Unit = performSuspending(__db, false, true) {
      _connection ->
    __updateAdapterOfSaga.handle(_connection, saga)
  }

  public override fun getAllSagas(): Flow<List<Saga>> {
    val _sql: String = "SELECT * FROM sagas"
    return createFlow(__db, false, arrayOf("sagas")) { _connection ->
      ...
      try {
        ...
        val _columnIndexOfMainCharacterId: Int = getColumnIndexOrThrow(_stmt, "mainCharacterId")
        val _columnIndexOfCurrentActId: Int = getColumnIndexOrThrow(_stmt, "currentActId")
        val _columnIndexOfIsEnded: Int = getColumnIndexOrThrow(_stmt, "isEnded")
        val _columnIndexOfEndedAt: Int = getColumnIndexOrThrow(_stmt, "endedAt")
        val _columnIndexOfIsDebug: Int = getColumnIndexOrThrow(_stmt, "isDebug")
        val _columnIndexOfEndMessage: Int = getColumnIndexOrThrow(_stmt, "endMessage")
        ...
        val _columnIndexOfPlaystyle: Int = getColumnIndexOrThrow(_stmt, "playstyle")
        val _columnIndexOfTopCharacters: Int = getColumnIndexOrThrow(_stmt, "topCharacters")
        val _columnIndexOfActsInsight: Int = getColumnIndexOrThrow(_stmt, "actsInsight")
        val _columnIndexOfConclusion: Int = getColumnIndexOrThrow(_stmt, "conclusion")
        val _result: MutableList<Saga> = mutableListOf()
        while (_stmt.step()) {
          val _item: Saga
          val _tmpId: Int
          ...
          val _tmpGenre: Genre
          _tmpGenre = __Genre_stringToEnum(_stmt.getText(_columnIndexOfGenre))
          val _tmpMainCharacterId: Int?
          if (_stmt.isNull(_columnIndexOfMainCharacterId)) {
            _tmpMainCharacterId = null
          } else {
            _tmpMainCharacterId = _stmt.getLong(_columnIndexOfMainCharacterId).toInt()
          }
          val _tmpCurrentActId: Int?
          if (_stmt.isNull(_columnIndexOfCurrentActId)) {
            _tmpCurrentActId = null
          } else {
            _tmpCurrentActId = _stmt.getLong(_columnIndexOfCurrentActId).toInt()
          }
          val _tmpIsEnded: Boolean
          val _tmp: Int
          _tmp = _stmt.getLong(_columnIndexOfIsEnded).toInt()
          _tmpIsEnded = _tmp != 0
          val _tmpEndedAt: Long
          _tmpEndedAt = _stmt.getLong(_columnIndexOfEndedAt)
          val _tmpIsDebug: Boolean
          val _tmp_1: Int
          _tmp_1 = _stmt.getLong(_columnIndexOfIsDebug).toInt()
          _tmpIsDebug = _tmp_1 != 0
          val _tmpEndMessage: String
          _tmpEndMessage = _stmt.getText(_columnIndexOfEndMessage)
          ...
          val _tmpReview: Review?
          if (!(_stmt.isNull(_columnIndexOfIntroduction) && _stmt.isNull(_columnIndexOfPlaystyle) &&
              _stmt.isNull(_columnIndexOfTopCharacters) && _stmt.isNull(_columnIndexOfActsInsight)
              && _stmt.isNull(_columnIndexOfConclusion))) {
            val _tmpIntroduction: String
            _tmpIntroduction = _stmt.getText(_columnIndexOfIntroduction)
            val _tmpPlaystyle: String
            _tmpPlaystyle = _stmt.getText(_columnIndexOfPlaystyle)
            val _tmpTopCharacters: String
            _tmpTopCharacters = _stmt.getText(_columnIndexOfTopCharacters)
            val _tmpActsInsight: String
            _tmpActsInsight = _stmt.getText(_columnIndexOfActsInsight)
            val _tmpConclusion: String
            _tmpConclusion = _stmt.getText(_columnIndexOfConclusion)
            _tmpReview =
                Review(_tmpIntroduction,_tmpPlaystyle,_tmpTopCharacters,_tmpActsInsight,_tmpConclusion)
          } else {
            _tmpReview = null
          }
          _item =
              Saga(_tmpId,_tmpTitle,_tmpDescription,_tmpIcon,_tmpCreatedAt,_tmpGenre,_tmpMainCharacterId,_tmpCurrentActId,_tmpIsEnded,_tmpEndedAt,_tmpIsDebug,_tmpEndMessage,_tmpReview,_tmpEmotionalReview)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override fun getSaga(sagaId: Int): Flow<Saga?> {
    val _sql: String = "SELECT * FROM sagas WHERE id = ?"
    return createFlow(__db, false, arrayOf("sagas")) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override fun getSagaContent(sagaId: Int): Flow<SagaContent?> {
    val _sql: String = "SELECT * FROM sagas WHERE id = ?"
    return createFlow(__db, true, arrayOf("Characters", "timelines", "character_events",
        "relationship_update_events", "character_relations", "reactions", "messages", "wikis",
        "Chapter", "acts", "sagas")) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
        _stmt.bindLong(_argIndex, sagaId.toLong())
        ...
        val _columnIndexOfActsInsight: Int = getColumnIndexOrThrow(_stmt, "actsInsight")
        val _columnIndexOfConclusion: Int = getColumnIndexOrThrow(_stmt, "conclusion")
        val _collectionMainCharacter: LongSparseArray<CharacterContent?> =
            LongSparseArray<CharacterContent?>()
        val _collectionCurrentActInfo: LongSparseArray<ActContent?> = LongSparseArray<ActContent?>()
        val _collectionCharacters: LongSparseArray<MutableList<CharacterContent>> =
            LongSparseArray<MutableList<CharacterContent>>()
        val _collectionWikis: LongSparseArray<MutableList<Wiki>> =
            LongSparseArray<MutableList<Wiki>>()
        val _collectionActs: LongSparseArray<MutableList<ActContent>> =
            LongSparseArray<MutableList<ActContent>>()
        val _collectionRelationships: LongSparseArray<MutableList<RelationshipContent>> =
        ...
        while (_stmt.step()) {
          ...
          if (_stmt.isNull(_columnIndexOfMainCharacterId)) {
            ...
            _tmpKey = _stmt.getLong(_columnIndexOfMainCharacterId)
          }
          if (_tmpKey != null) {
            _collectionMainCharacter.put(_tmpKey, null)
          }
          val _tmpKey_1: Long?
          if (_stmt.isNull(_columnIndexOfCurrentActId)) {
            _tmpKey_1 = null
          } else {
            _tmpKey_1 = _stmt.getLong(_columnIndexOfCurrentActId)
          }
          if (_tmpKey_1 != null) {
            _collectionCurrentActInfo.put(_tmpKey_1, null)
          }
          val _tmpKey_2: Long
          _tmpKey_2 = _stmt.getLong(_columnIndexOfId)
          if (!_collectionCharacters.containsKey(_tmpKey_2)) {
            _collectionCharacters.put(_tmpKey_2, mutableListOf())
          }
          val _tmpKey_3: Long
          _tmpKey_3 = _stmt.getLong(_columnIndexOfId)
          if (!_collectionWikis.containsKey(_tmpKey_3)) {
            _collectionWikis.put(_tmpKey_3, mutableListOf())
          }
          val _tmpKey_4: Long
          _tmpKey_4 = _stmt.getLong(_columnIndexOfId)
          if (!_collectionActs.containsKey(_tmpKey_4)) {
            _collectionActs.put(_tmpKey_4, mutableListOf())
          }
          val _tmpKey_5: Long
          _tmpKey_5 = _stmt.getLong(_columnIndexOfId)
          if (!_collectionRelationships.containsKey(_tmpKey_5)) {
            _collectionRelationships.put(_tmpKey_5, mutableListOf())
          }
        }
        _stmt.reset()
        __fetchRelationshipCharactersAscomIlustrisSagaiFeaturesCharactersDataModelCharacterContent(_connection,
            _collectionMainCharacter)
        __fetchRelationshipactsAscomIlustrisSagaiFeaturesActDataModelActContent(_connection,
            _collectionCurrentActInfo)
        __fetchRelationshipCharactersAscomIlustrisSagaiFeaturesCharactersDataModelCharacterContent_1(_connection,
            _collectionCharacters)
        __fetchRelationshipwikisAscomIlustrisSagaiFeaturesWikiDataModelWiki_1(_connection,
            _collectionWikis)
        __fetchRelationshipactsAscomIlustrisSagaiFeaturesActDataModelActContent_1(_connection,
            _collectionActs)
        __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_3(_connection,
            _collectionRelationships)
        val _result: SagaContent?
        if (_stmt.step()) {
          val _tmpData: Saga
          val _tmpId: Int
          _tmpId = _stmt.getLong(_columnIndexOfId).toInt()
          ...
          _tmpData =
              Saga(_tmpId,_tmpTitle,_tmpDescription,_tmpIcon,_tmpCreatedAt,_tmpGenre,_tmpMainCharacterId,_tmpCurrentActId,_tmpIsEnded,_tmpEndedAt,_tmpIsDebug,_tmpEndMessage,_tmpReview,_tmpEmotionalReview)
          val _tmpMainCharacter: CharacterContent?
          val _tmpKey_6: Long?
          if (_stmt.isNull(_columnIndexOfMainCharacterId)) {
            _tmpKey_6 = null
          } else {
            _tmpKey_6 = _stmt.getLong(_columnIndexOfMainCharacterId)
          }
          if (_tmpKey_6 != null) {
            _tmpMainCharacter = _collectionMainCharacter.get(_tmpKey_6)
          } else {
            _tmpMainCharacter = null
          }
          val _tmpCurrentActInfo: ActContent?
          val _tmpKey_7: Long?
          if (_stmt.isNull(_columnIndexOfCurrentActId)) {
            _tmpKey_7 = null
          } else {
            _tmpKey_7 = _stmt.getLong(_columnIndexOfCurrentActId)
          }
          if (_tmpKey_7 != null) {
            _tmpCurrentActInfo = _collectionCurrentActInfo.get(_tmpKey_7)
          } else {
            _tmpCurrentActInfo = null
          }
          val _tmpCharactersCollection: MutableList<CharacterContent>
          val _tmpKey_8: Long
          _tmpKey_8 = _stmt.getLong(_columnIndexOfId)
          _tmpCharactersCollection = checkNotNull(_collectionCharacters.get(_tmpKey_8))
          val _tmpWikisCollection: MutableList<Wiki>
          val _tmpKey_9: Long
          _tmpKey_9 = _stmt.getLong(_columnIndexOfId)
          _tmpWikisCollection = checkNotNull(_collectionWikis.get(_tmpKey_9))
          val _tmpActsCollection: MutableList<ActContent>
          val _tmpKey_10: Long
          _tmpKey_10 = _stmt.getLong(_columnIndexOfId)
          _tmpActsCollection = checkNotNull(_collectionActs.get(_tmpKey_10))
          val _tmpRelationshipsCollection: MutableList<RelationshipContent>
          val _tmpKey_11: Long
          _tmpKey_11 = _stmt.getLong(_columnIndexOfId)
          _tmpRelationshipsCollection = checkNotNull(_collectionRelationships.get(_tmpKey_11))
          _result =
              SagaContent(_tmpData,_tmpMainCharacter,_tmpCurrentActInfo,_tmpCharactersCollection,_tmpWikisCollection,_tmpActsCollection,_tmpRelationshipsCollection)
        } else {
          _result = null
        }
        _result
  ...
  public override fun getSagaContent(): Flow<List<SagaContent>> {
    val _sql: String = "SELECT * FROM sagas"
    return createFlow(__db, true, arrayOf("Characters", "timelines", "character_events",
        "relationship_update_events", "character_relations", "reactions", "messages", "wikis",
        "Chapter", "acts", "sagas")) { _connection ->
      ...
      try {
        ...
        __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_3(_connection,
            _collectionRelationships)
        val _result: MutableList<SagaContent> = mutableListOf()
        while (_stmt.step()) {
          val _item: SagaContent
          val _tmpData: Saga
          val _tmpId: Int
          _tmpId = _stmt.getLong(_columnIndexOfId).toInt()
          val _tmpTitle: String
  ...
  public override suspend fun deleteSagaData(sagaId: String) {
    val _sql: String = "DELETE FROM sagas WHERE id = ?"
  ...
  public override suspend fun deleteAllSagas() {
    val _sql: String = "DELETE FROM sagas"
    return performSuspending(__db, false, true) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        _stmt.step()
  ...
  private fun __Genre_enumToString(_value: Genre): String = when (_value) {
    Genre.FANTASY -> "FANTASY"
    Genre.CYBERPUNK -> "CYBERPUNK"
    Genre.HORROR -> "HORROR"
    Genre.HEROES -> "HEROES"
    Genre.CRIME -> "CRIME"
    Genre.SHINOBI -> "SHINOBI"
    Genre.SPACE_OPERA -> "SPACE_OPERA"
  }

  private fun __Genre_stringToEnum(_value: String): Genre = when (_value) {
    "FANTASY" -> Genre.FANTASY
    "CYBERPUNK" -> Genre.CYBERPUNK
    "HORROR" -> Genre.HORROR
    "HEROES" -> Genre.HEROES
    "CRIME" -> Genre.CRIME
    "SHINOBI" -> Genre.SHINOBI
    "SPACE_OPERA" -> Genre.SPACE_OPERA
  ...
  private
    ...
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`characterId`,`gameTimelineId`,`title`,`summary`,`createdAt` FROM `character_events` WHERE `characterId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
    ...
    try {
      val _itemKeyIndex: Int = getColumnIndex(_stmt, "characterId")
      if (_itemKeyIndex == -1) {
        return
      }
      val _columnIndexOfId: Int = 0
  ...
  private
    ...
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`characterOneId`,`characterTwoId`,`sagaId`,`title`,`description`,`emoji`,`lastUpdated` FROM `character_relations` WHERE `characterOneId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
    ...
    try {
      val _itemKeyIndex: Int = getColumnIndex(_stmt, "characterOneId")
      if (_itemKeyIndex == -1) {
        return
      }
      val _columnIndexOfId: Int = 0
  ...
  private
      fun __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_1(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<RelationshipContent>>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
      recursiveFetchLongSparseArray(_map, true) { _tmpMap ->
        __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_1(_connection,
            _tmpMap)
      }
      return
    }
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`characterOneId`,`characterTwoId`,`sagaId`,`title`,`description`,`emoji`,`lastUpdated` FROM `character_relations` WHERE `characterTwoId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
    ...
    try {
      val _itemKeyIndex: Int = getColumnIndex(_stmt, "characterTwoId")
      if (_itemKeyIndex == -1) {
        return
      }
      val _columnIndexOfId: Int = 0
  ...
  private
      fun __fetchRelationshipCharactersAscomIlustrisSagaiFeaturesCharactersDataModelCharacterContent(_connection: SQLiteConnection,
      _map: LongSparseArray<CharacterContent?>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
    ...
    try {
      ...
      val _columnIndexOfOccupation: Int = 27
      val _columnIndexOfPersonality: Int = 28
      val _collectionEvents: LongSparseArray<MutableList<CharacterEventDetails>> =
          LongSparseArray<MutableList<CharacterEventDetails>>()
      val _collectionRelationshipsAsFirst: LongSparseArray<MutableList<RelationshipContent>> =
          LongSparseArray<MutableList<RelationshipContent>>()
      val _collectionRelationshipsAsSecond: LongSparseArray<MutableList<RelationshipContent>> =
          LongSparseArray<MutableList<RelationshipContent>>()
      while (_stmt.step()) {
        val _tmpKey: Long
        ...
        if (!_collectionRelationshipsAsFirst.containsKey(_tmpKey_1)) {
          _collectionRelationshipsAsFirst.put(_tmpKey_1, mutableListOf())
        }
        val _tmpKey_2: Long
        _tmpKey_2 = _stmt.getLong(_columnIndexOfId)
        if (!_collectionRelationshipsAsSecond.containsKey(_tmpKey_2)) {
          _collectionRelationshipsAsSecond.put(_tmpKey_2, mutableListOf())
        }
      }
      _stmt.reset()
      __fetchRelationshipcharacterEventsAscomIlustrisSagaiFeaturesCharactersEventsDataModelCharacterEventDetails(_connection,
          _collectionEvents)
      __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent(_connection,
          _collectionRelationshipsAsFirst)
      __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_1(_connection,
          _collectionRelationshipsAsSecond)
      while (_stmt.step()) {
        val _tmpKey_3: Long
        _tmpKey_3 = _stmt.getLong(_itemKeyIndex)
        if (_map.containsKey(_tmpKey_3)) {
          val _item_1: CharacterContent?
          val _tmpData: Character
          val _tmpId: Int
          _tmpId = _stmt.getLong(_columnIndexOfId).toInt()
          val _tmpName: String
          _tmpName = _stmt.getText(_columnIndexOfName)
          ...
          _tmpData =
              Character(_tmpId,_tmpName,_tmpBackstory,_tmpImage,_tmpHexColor,_tmpSagaId,_tmpDetails,_tmpProfile,_tmpJoinedAt,_tmpFirstSceneId,_tmpEmojified)
          val _tmpEventsCollection: MutableList<CharacterEventDetails>
          val _tmpKey_4: Long
          _tmpKey_4 = _stmt.getLong(_columnIndexOfId)
          _tmpEventsCollection = checkNotNull(_collectionEvents.get(_tmpKey_4))
          val _tmpRelationshipsAsFirstCollection: MutableList<RelationshipContent>
          val _tmpKey_5: Long
          _tmpKey_5 = _stmt.getLong(_columnIndexOfId)
          _tmpRelationshipsAsFirstCollection =
              checkNotNull(_collectionRelationshipsAsFirst.get(_tmpKey_5))
          val _tmpRelationshipsAsSecondCollection: MutableList<RelationshipContent>
          val _tmpKey_6: Long
          _tmpKey_6 = _stmt.getLong(_columnIndexOfId)
          _tmpRelationshipsAsSecondCollection =
              checkNotNull(_collectionRelationshipsAsSecond.get(_tmpKey_6))
          _item_1 =
              CharacterContent(_tmpData,_tmpEventsCollection,_tmpRelationshipsAsFirstCollection,_tmpRelationshipsAsSecondCollection)
          _map.put(_tmpKey_3, _item_1)
        }
      }
    } finally {
      _stmt.close()
  ...
  private
      fun __fetchRelationshipcharacterEventsAscomIlustrisSagaiFeaturesCharactersEventsDataModelCharacterEventDetails_1(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<CharacterEventDetails>>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
      recursiveFetchLongSparseArray(_map, true) { _tmpMap ->
        __fetchRelationshipcharacterEventsAscomIlustrisSagaiFeaturesCharactersEventsDataModelCharacterEventDetails_1(_connection,
            _tmpMap)
      }
      return
    }
  ...
  private
      fun __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_2(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<RelationshipContent>>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
      recursiveFetchLongSparseArray(_map, true) { _tmpMap ->
        __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_2(_connection,
            _tmpMap)
      }
      return
    }
  ...
  private
      fun __fetchRelationshipactsAscomIlustrisSagaiFeaturesActDataModelActContent(_connection: SQLiteConnection,
      _map: LongSparseArray<ActContent?>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
    ...
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`title`,`content`,`introduction`,`emotionalReview`,`sagaId`,`currentChapterId` FROM `acts` WHERE `id` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
    ...
    try {
      ...
      val _columnIndexOfEmotionalReview: Int = 4
      val _columnIndexOfSagaId: Int = 5
      val _columnIndexOfCurrentChapterId: Int = 6
      ...
      while (_stmt.step()) {
        ...
        if (_map.containsKey(_tmpKey_2)) {
          ...
          if (_stmt.isNull(_columnIndexOfCurrentChapterId)) {
            ...
            _tmpKey_3 = _stmt.getLong(_columnIndexOfCurrentChapterId)
          }
          if (_tmpKey_3 != null) {
            _tmpCurrentChapterInfo = _collectionCurrentChapterInfo.get(_tmpKey_3)
          } else {
          ...
          val _tmpKey_4: Long
          _tmpKey_4 = _stmt.getLong(_columnIndexOfId)
          _tmpChaptersCollection = checkNotNull(_collectionChapters.get(_tmpKey_4))
          _item_1 = ActContent(_tmpData,_tmpCurrentChapterInfo,_tmpChaptersCollection)
          _map.put(_tmpKey_2, _item_1)
        }
      }
    } finally {
  ...
  private
      fun __fetchRelationshipCharactersAscomIlustrisSagaiFeaturesCharactersDataModelCharacterContent_1(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<CharacterContent>>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
    ...
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`name`,`backstory`,`image`,`hexColor`,`sagaId`,`joinedAt`,`firstSceneId`,`emojified`,`race`,`gender`,`ethnicity`,`height`,`weight`,`hair`,`eyes`,`mouth`,`distinctiveMarks`,`jawline`,`buildAndPosture`,`skinAppearance`,`distinguishFeatures`,`outfitDescription`,`accessories`,`carriedItems`,`skillsAndProficiencies`,`uniqueOrSignatureTalents`,`occupation`,`personality` FROM `Characters` WHERE `sagaId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
    ...
    try {
      val _itemKeyIndex: Int = getColumnIndex(_stmt, "sagaId")
      if (_itemKeyIndex == -1) {
        return
      }
      val _columnIndexOfId: Int = 0
      ...
      while (_stmt.step()) {
        val _tmpKey_3: Long
        _tmpKey_3 = _stmt.getLong(_itemKeyIndex)
        val _tmpRelation: MutableList<CharacterContent>? = _map.get(_tmpKey_3)
        if (_tmpRelation != null) {
          val _item_1: CharacterContent
          val _tmpData: Character
          val _tmpId: Int
  ...
  private
      fun __fetchRelationshipwikisAscomIlustrisSagaiFeaturesWikiDataModelWiki_1(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<Wiki>>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
      recursiveFetchLongSparseArray(_map, true) { _tmpMap ->
        __fetchRelationshipwikisAscomIlustrisSagaiFeaturesWikiDataModelWiki_1(_connection, _tmpMap)
      }
      return
    }
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`title`,`content`,`type`,`emojiTag`,`createdAt`,`sagaId`,`timelineId` FROM `wikis` WHERE `sagaId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
  ...
  private
      fun __fetchRelationshipactsAscomIlustrisSagaiFeaturesActDataModelActContent_1(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<ActContent>>) {
    if (_map.isEmpty()) {
      return
    }
    if (_map.size() > 999) {
    ...
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`title`,`content`,`introduction`,`emotionalReview`,`sagaId`,`currentChapterId` FROM `acts` WHERE `sagaId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
    ...
    try {
      ...
      while (_stmt.step()) {
        ...
        if (_tmpKey_2 != null) {
          val _tmpRelation: MutableList<ActContent>? = _map.get(_tmpKey_2)
          if (_tmpRelation != null) {
            val _item_1: ActContent
            val _tmpData: Act
            val _tmpId: Int
  ...
  private
      fun __fetchRelationshipcharacterRelationsAscomIlustrisSagaiFeaturesCharactersRelationsDataModelRelationshipContent_3(_connection: SQLiteConnection,
      _map: LongSparseArray<MutableList<RelationshipContent>>) {
    if (_map.isEmpty()) {
      return
    }
    ...
    val _stringBuilder: StringBuilder = StringBuilder()
    _stringBuilder.append("SELECT `id`,`characterOneId`,`characterTwoId`,`sagaId`,`title`,`description`,`emoji`,`lastUpdated` FROM `character_relations` WHERE `sagaId` IN (")
    val _inputSize: Int = _map.size()
    appendPlaceholders(_stringBuilder, _inputSize)
    _stringBuilder.append(")")
    val _sql: String = _stringBuilder.toString()
