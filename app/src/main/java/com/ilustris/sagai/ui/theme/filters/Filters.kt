...
@Composable
fun Modifier.ultimateEnergyShader(
    isPlaying: Boolean = true,
    energyColor: Color = Color(0xFF8CE8FF),
    outerColor: Color = Color(0xFF2666FF),
    intensity: Float = 0.9f,
    width: Float = 10f,
    wispIntensity: Float = 0.75f,
    wispSpeed: Float = 3f,
): Modifier =
    composed {
        ...
        val agslShaderSource = loadShaderFromAssetsOnce("genre_filter_shader.agsl")
        if (agslShaderSource == null) return@composed this
        ...
        val uniformValues =
            remember(energyColor, outerColor, intensity, width, wispIntensity, wispSpeed) {
                ShaderParams(
                    saturation = 1f,
                    contrast = 1f,
                    rimEnergyIntensity = intensity,
                    rimEnergyColor = Triple(energyColor.red, energyColor.green, energyColor.blue),
                    rimEnergyOuterColor = Triple(outerColor.red, outerColor.green, outerColor.blue),
                    rimEnergyWidth = width,
